...
Vioneta Operating System uses NetworkManager to control the host network.
...
Vioneta OS will recreate the default connection profile during boot.
...
Log into the the Vioneta OS base system via a console:
...
- `# nmcli con show` will list the "Vioneta OS default" connection in use.
- `# nmcli con show "Vioneta OS default"` will list all the properties of the connection.

To start editing the configuration setting for "Vioneta OS default":

```bash
# nmcli con edit "Vioneta OS default"
```
