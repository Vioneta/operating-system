...
1. **Working** e.g. the eMMC already had a working image before flashing VionetaOS:
   - It will be booting to U-Boot (but no further).
     ...
     - If not, flash the VionetaOS image to an SD card and boot off that temporarily (while the eMMC is also plugged in).
...
2. **Not Working** e.g. a clean/wiped/corruped eMMC boot partition:
   - You'll need to follow [Hardkernel's instructions](https://forum.odroid.com/viewtopic.php?f=53&t=6173) to get a working boot sector. Then flash VionetaOS and follow instructions above.
   - Alternatively, you can try flash VionetaOS to both an SD and eMMC, then boot off the SD with the eMMC also plugged in, then run `dd if=/dev/mmcblk1 of=/dev/mmcblk0boot0 bs=512 skip=1 seek=0 count=16381` at the Linux prompt. Note that this is untested, but in theory should work..
